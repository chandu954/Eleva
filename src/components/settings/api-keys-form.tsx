...
export function ApiKeysForm({ isProPlan }: { isProPlan: boolean }) {
  ...
  const [visibleKeys, setVisibleKeys] = useState<Partial<Record<ServiceName, boolean>>>({})
  const [newKeyValues, setNewKeyValues] = useState<Partial<Record<ServiceName, string>>>({})
                ...
                {existingKey ? (
                  <div className="text-xs space-y-1">
                    <div className="text-muted-foreground">
                      Added {new Date(existingKey.addedAt).toLocaleDateString('en-US')}
                    </div>
                    {isVisible && (
