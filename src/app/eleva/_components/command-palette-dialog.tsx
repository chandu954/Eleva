...
export function CommandPaletteDialog({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-start justify-center pt-[12vh] px-4" style={{ background: 'rgba(2,6,23,0.45)', backdropFilter: 'blur(6px)' }} onClick={onClose} data-testid="command-palette-overlay">
          <motion.div initial={{ opacity: 0, y: -12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ type: 'spring', stiffness: 300, damping: 26 }} className="w-full max-w-2xl rounded-2xl overflow-hidden" style={{ background: 'rgb(var(--eleva-card))', border: '1px solid rgb(var(--eleva-border))', boxShadow: '0 30px 80px -20px rgba(2,6,23,0.35)' }} onClick={(e) => e.stopPropagation()} data-testid="command-palette-modal">
                ...
                grouped.map(([groupName, groupRows]) => (
                  <div key={groupName} className="mb-2">
                    ...
                    {groupRows.map((item) => {
                      ...
                      return (
                        <button key={idx} onMouseEnter={() => setSelected(idx)} onClick={() => go(item.href)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors" style={{ background: active ? 'rgb(var(--eleva-muted))' : 'transparent' }}>
                          <item.icon className="w-4 h-4 shrink-0" style={{ color: active ? 'rgb(var(--eleva-primary))' : 'rgb(var(--eleva-muted-fg))' }} />
                          <span className="flex-1 min-w-0">
                            <span className="block text-[13px] truncate" style={{ color: 'rgb(var(--eleva-fg))' }}>{item.label}</span>
                            {item.sub && <span className="block text-[11px] truncate" style={{ color: 'rgb(var(--eleva-muted-fg))' }}>{item.sub}</span>}
                          </span>
                          {item.badge && <span className="text-[10px] font-mono uppercase px-2 py-0.5 rounded-full" style={{ background: 'rgb(var(--eleva-muted))', color: 'rgb(var(--eleva-muted-fg))' }}>{item.badge}</span>}
                          {item.hint && <span className="eleva-kbd shrink-0">{item.hint}</span>}
                        </button>
                      );
                    })}
                  </div>
            ...
            <div className="px-4 h-10 flex items-center justify-between border-t" style={{ borderColor: 'rgb(var(--eleva-border))' }}>
              <span className="text-[11px]" style={{ color: 'rgb(var(--eleva-muted-fg))' }}>{rows.length} results</span>
              <span className="text-[11px] flex items-center gap-3" style={{ color: 'rgb(var(--eleva-muted-fg))' }}>
                <span><span className="eleva-kbd">↑↓</span> navigate</span>
                <span><span className="eleva-kbd">↵</span> open</span>
              </span>
            </div>
          </motion.div>
        </motion.div>
